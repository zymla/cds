...
# FAIRE CHOISIR LA DISTANCE METRIQUE : "euclidean", "manhattan", "jaccard" (on créera la variable distm)
...
# Je n'ai as réussi à ajouter dans la fonction aes, après colr = cl_clara, les infos pour afficher pour chaque point le numéro de la classe, nom de la station, longitude, latitude
# Ligne de code qui marche pas
...
# FAIRE CHOISIR LE NOMBRE DE CLASSES : entre 1 et 15 (on créera la variable nbcl)
# FAIRE CHOISIR LE NOMBRES D'AXES A CONSERVER : 50%, 60%, 70%, 80%, 90%, 100% (on créera la # variable choixvp qui variera entre 1 (pour 50%) et 11 pour 100% 
...
# Nombre d'axes issues de l'ACP à conserver pour la CAH
# On réalise l'ACP avec l'ensemble des axes et on récupère la distribution des vp
...
# On réalise l'ACP avec l'ensemble des axes en ne conservant que le nb d'axes choisis (paramètre nbvp)
pca <- PCA(scale(trajets_cl,center=T,scale=T),ncp=nbvp,graph=FALSE)

# Pour la CAH on donne en paramètre le nombre de classes (nbcl), le nb d'axes à conserver (nbvp)
...
print(paste("Inertie totale conservée : ",vp[obs==nbvp,Cumvp]))
print(paste("Nombre de classes optimales suggéré : ",cah_clopt$call$t$nb.clust))
rm(cah_clopt)
rm(choixvp,nbvp,vp)
...
# Je n'ai as réussi à ajouter dans la fonction aes, après colr = cl_pca, les infos pour afficher pour chaque point le numéro de la classe, nom de la station, longitude, latitude
# Ligne de code qui marche pas
