...
station_movements_kmeans_input <- 
  station_movements[wday_n == 3 & type == 'from', .(hr = as.factor(hr) %>% fct_shift(5), N, dailyN = sum(N)), station_id][, .(N = N/dailyN, dailyN, station_id, hr)] %>% 
  dcast(station_id + dailyN ~ hr, fill = 0, value.var = 'N')
station_movements_kmeans_input[]
```
```{r}
...
```
```{r}
station_movements[wday_n == 3 & type == 'from', .(hr = as.factor(hr) %>% fct_shift(5), N, dailyN = sum(N)), station_id][, .(N = N/dailyN, dailyN, station_id, hr)][stationsKM[,.(station_id, classeKM)], on = 'station_id'] %>% 
  {
    ggplot() +
#geom_point(data = .[,.(longitude, latitude)], aes(longitude, latitude), colour = 'gray') +
