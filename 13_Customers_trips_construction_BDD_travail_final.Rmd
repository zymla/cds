...
- l'écriture, la lecture puis la suppression des 3 tables : stations_15min, stations_4h, stations_h
Change (obligée sinon Rstudio sature en mémoire)
- l'écritude sur le disque des deux tables de travail pour Shiny: trajets et des_trajets
...
Création d'une table annuaire des stations uniformisées 
On contrôle des cas où le nom change dans le temps de même que la localistaion géographique
On conserve pour chaque station les dernières valeurs observées pour la station
...
#### L'idée est de calculer pour chaque station le nb de stations à 300m, 1km...
...
#### Gestion des heures
###  On va construire des tranches horaires pour apparier l'activité des stations avec les données de trajets
...
# Aggrégation par station, date (mois, jour), et tranches horaires
...
Suppression des caratéristiques des subscribers 
...
### Nb de trips partants et arrivant de/à chaque station
...
st_trips <- trajets[,c("from_station_id","to_station_id","trip_id","start_jour","start_mois","wday_n","start_time")]
  ...
  # qu'une journée commence à 5 heures. 
  # En pratique c'est 1.5% des obs... mais comme ensuite on agrège sur e jour de la semaine 
  # on peut considérer que c'est mineur
...
### Nb de vélos différents partants et arrivant de/à chaque station
...
#### On ajoute : les flux de cyclistes arrivants et partants et le nb de vélos différents sur la journée
...
```{r}
  # on remplace les potentielles NA restantes : valeur moyenne sur la journée
...
trajets <- na.omit(trajets) 
    # quelques cas où distance non calculable, valeurs manquantes sur tx de remplissage: on les supprime
    # 390 231 obs à 390 145 trajets
